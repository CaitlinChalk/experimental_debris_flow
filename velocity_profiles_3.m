...
%% mu(I) rheology scaling

%shear rate
% shear_rate = cell(1,n2);
% pressure = cell(1,n2);
% I = cell(1,n2); %Inertia number
% A = cell(1,n2); %mu(I) rheology parameter 
rho = 1373.3;
theta = 31;
...
for i = 1:n2
   ...
   % xlim([-inf max(U)+0.1])
%     if i == n2
% %         legend(["Exp. data","$\mu (I)$ scaling"],'Interpreter','Latex','Location','EastOutside','FontSize',10)
% %         ylim([0 max(Y2)+0.0012])
%     end
%      figure(2); hold on
%      check = 1./((pressure(2:end)).^0.5);
%      %check = A.*(h^(3/2) - (h - Y2(2:end)).^(3/2));
%    %  plot(check,Y2(2:end),'-o')
%     plot(Y2(2:end),UmuI)
end
% figure(2)
% xline(0.1,'Linewidth',2);
% xlim([-2 2])
%legend(legend_entry{1:n2},'Location','SouthWest','Interpreter','Latex','FontSize',12);
